...
evrle_structure_read_marker_impl(struct level_parser *const self,
...
static enum token evrle_structure_read_key(struct level_parser *const self,
                                           struct dicm_src *src) {
  struct dual dual;
  const enum token token = evrle_structure_read_marker_impl(self, src, &dual);
  if (token != TOKEN_KEY)
    return token;
...
static enum token evrle_level_parser_read_value(struct level_parser *self,
                                                struct dicm_src *src) {
  assert(src);
...
static enum state evrle_root_parser_next_event(struct level_parser *self,
...
static enum state evrle_level_parser_next_event(struct level_parser *self,
                                                const enum state current_state,
                                                struct dicm_src *src) {
  enum token token;
  enum state new_state = STATE_INVALID;
...
static enum token evrle_structure_write_key(struct level_emitter *self,
                                            struct dicm_dst *dst,
...
static enum token
evrle_level_emitter_write_vl(struct level_emitter *self, struct dicm_dst *dst,
                             const enum dicm_event_type next) {
...
static enum token
evrle_level_emitter_write_value(struct level_emitter *self,
                                struct dicm_dst *dst,
                                const enum dicm_event_type next) {
  enum token token = TOKEN_INVALID_DATA;
  switch (next) {
...
static enum state evrle_root_emitter_next_event(
...
static enum state evrle_level_emitter_next_event(
    struct level_emitter *self, const enum state current_state,
    struct dicm_dst *dst, const enum dicm_event_type next) {
  enum token token;
  enum state new_state = STATE_INVALID;
...
evrle_level_parser_next_level(struct level_parser *level_parser,
                              const enum state current_state);

static struct level_parser_vtable const evrle_ds_vtable = {
    /* ds reader interface */
    .reader = {.fp_structure_read_key = evrle_structure_read_key,
               .fp_value_token = evrle_level_parser_read_value,
               .fp_next_level = evrle_level_parser_next_level,
               .fp_next_event = evrle_root_parser_next_event}};
static struct level_parser_vtable const evrle_item_vtable = {
    /* item reader interface */
    .reader = {.fp_structure_read_key = evrle_structure_read_key,
               .fp_value_token = evrle_level_parser_read_value,
               .fp_next_level = evrle_level_parser_next_level,
               .fp_next_event = evrle_level_parser_next_event}};

struct level_parser
evrle_level_parser_next_level(struct level_parser *self,
                              const enum state current_state) {
  assert(STATE_STARTSEQUENCE == current_state);
  struct level_parser new_item = {.vtable = &evrle_item_vtable};
  sequence_setup_next_level(self, &new_item);
  return new_item;
}

struct level_parser get_new_evrle_reader_ds() {
  struct level_parser new_item = {.vtable = &evrle_ds_vtable};
  sequence_setup_level(&new_item);
  return new_item;
}

static struct level_emitter
evrle_level_emitter_next_level(struct level_emitter *self,
                               const enum state current_state);

static struct level_emitter_vtable const g_evrle_root_vtable = {
    /* root emitter interface */
    .level_emitter = {.fp_structure_key_token = evrle_structure_write_key,
                      .fp_vl_token = evrle_level_emitter_write_vl,
                      .fp_value_token = evrle_level_emitter_write_value,
                      .fp_next_level = evrle_level_emitter_next_level,
                      .fp_next_event = evrle_root_emitter_next_event}};
static struct level_emitter_vtable const g_evrle_item_vtable = {
    /* item level emitter interface */
    .level_emitter = {.fp_structure_key_token = evrle_structure_write_key,
                      .fp_vl_token = evrle_level_emitter_write_vl,
                      .fp_value_token = evrle_level_emitter_write_value,
                      .fp_next_level = evrle_level_emitter_next_level,
                      .fp_next_event = evrle_level_emitter_next_event}};

struct level_emitter
evrle_level_emitter_next_level(struct level_emitter *self,
                               const enum state current_state) {
  assert(STATE_STARTSEQUENCE == current_state);
  struct level_emitter new_item = {.vtable = &g_evrle_item_vtable};
...
void evrle_init_level_emitter(struct level_emitter *const new_item) {
  ...
  new_item->vtable = &g_evrle_root_vtable;
