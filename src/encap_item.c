...
static enum dicm_token encap_item_reader_read_key(struct _item_reader *self,
                                                  struct dicm_src *src) {
  struct dual dual;
...
static enum dicm_token encap_item_reader_read_value(struct _item_reader *self,
                                                    struct dicm_src *src) {
  assert(src);
...
encap_root_reader_next_event(struct _item_reader *self,
...
static enum dicm_state
encap_item_reader_next_event(struct _item_reader *self,
                             const enum dicm_state current_state,
                             struct dicm_src *src) {
  enum dicm_token next;
  enum dicm_state new_state = STATE_INVALID;
...
static enum dicm_state
encap_item_writer_write_key(struct _item_writer *self, struct dicm_dst *dst,
...
static enum dicm_state
encap_item_writer_write_value(struct _item_writer *self, struct dicm_dst *dst,
                              const enum dicm_token token) {
...
static enum dicm_state encap_root_writer_next_event(
...
static enum dicm_state encap_item_writer_next_event(
    struct _item_writer *self, const enum dicm_state current_state,
    struct dicm_dst *dst, const enum dicm_event_type next) {
  const enum dicm_token token = event2token(next);
  enum dicm_state new_state = STATE_INVALID;
...
static enum dicm_token encap_frag_reader_read_value(struct _item_reader *self,
                                                    struct dicm_src *src) {
  assert(src);
  const dicm_vr_t vr = self->da.vr;
  assert(vr == VR_NONE);
...
static enum dicm_token encap_frag_reader_read_key(struct _item_reader *self,
                                                  struct dicm_src *src) {
  struct dual dual;
  int64_t ssize = dicm_src_read(src, &dual.ivr, 8);
  if (ssize != 8) {
 ...
 */
static enum dicm_state
encap_frag_reader_next_event(struct _item_reader *self,
                             const enum dicm_state current_state,
                             struct dicm_src *src) {
  enum dicm_token next;
  enum dicm_state new_state = STATE_INVALID;
...
static enum dicm_state
encap_frag_writer_write_key(struct _item_writer *self, struct dicm_dst *dst,
                            const enum dicm_token token) {
  enum dicm_state new_state = STATE_INVALID;
  int64_t dlen;
  switch (token) {
...
static enum dicm_state
encap_frag_writer_write_value(struct _item_writer *self, struct dicm_dst *dst,
                              const enum dicm_token token) {
  assert(token == TOKEN_VALUE);
  enum dicm_state new_state = STATE_INVALID;
  int64_t dlen;
...
static enum dicm_state encap_frag_writer_next_event(
    struct _item_writer *self, const enum dicm_state current_state,
    struct dicm_dst *dst, const enum dicm_event_type next) {
  const enum dicm_token token = event2token(next);
  enum dicm_state new_state = STATE_INVALID;
...
encap_item_reader_next_level(struct _item_reader *item_reader,
                             const enum dicm_state current_state);

static struct _item_reader_vtable const encap_root_vtable = {
    /* ds reader interface */
    .reader = {.fp_key_token = encap_item_reader_read_key,
               .fp_value_token = encap_item_reader_read_value,
               .fp_next_level = encap_item_reader_next_level,
               .fp_next_event = encap_root_reader_next_event}};
static struct _item_reader_vtable const encap_item_vtable = {
    /* item reader interface */
    .reader = {.fp_key_token = encap_item_reader_read_key,
               .fp_value_token = encap_item_reader_read_value,
               .fp_next_level = encap_item_reader_next_level,
               .fp_next_event = encap_item_reader_next_event}};
static struct _item_reader_vtable const encap_frag_vtable = {
...
struct _item_reader get_new_reader_ds() {
  struct _item_reader new_item = {.vtable = &encap_root_vtable};
  return new_item;
}

struct _item_reader get_new_reader_item() {
  struct _item_reader new_item = {.vtable = &encap_item_vtable};
  return new_item;
}

struct _item_reader get_new_reader_frag() {
  struct _item_reader new_item = {.vtable = &encap_frag_vtable};
  return new_item;
}

static struct _item_writer
encap_item_writer_next_level(struct _item_writer *self,
                             const enum dicm_state current_state);

static struct _item_writer_vtable const g_encap_root_vtable = {
    /* ds writer interface */
    .writer = {.fp_key_token = encap_item_writer_write_key,
               .fp_vl_token = encap_item_writer_write_vl,
               .fp_value_token = encap_item_writer_write_value,
               .fp_next_level = encap_item_writer_next_level,
               .fp_next_event = encap_root_writer_next_event}};
static struct _item_writer_vtable const g_encap_item_vtable = {
    /* item writer interface */
    .writer = {.fp_key_token = encap_item_writer_write_key,
               .fp_vl_token = encap_item_writer_write_vl,
               .fp_value_token = encap_item_writer_write_value,
               .fp_next_level = encap_item_writer_next_level,
               .fp_next_event = encap_item_writer_next_event}};
static struct _item_writer_vtable const g_encap_frag_vtable = {
    /* fragment writer interface */
    .writer = {.fp_key_token = encap_frag_writer_write_key,
               .fp_vl_token = encap_item_writer_write_vl,
